...
def query_bs_db(db: Session, name: str, startDate: str = None, endDate: str = None):
    ...
    # Aggregate totals
    total_pa = total_ab = total_hr = total_h = total_b1 = total_b2 = total_b3 = total_rbi = total_r = 0
    total_bb = total_so = total_nump = 0
    total_sb = total_cs = 0

    for s in stats:
        print("stat date is " + s.gameId + "r: " + str(s.rr) + " " + str(s.lr))
    ...
    obp = (total_h + total_bb) / (total_ab + total_bb) if (total_ab + total_bb) else 0
