from app.models import BatterDailyStats, BatterYearlyStats, Player
...
def sample_players(player_list, all_player_ats, league_ba, print_players=True, eval_prv=True, ab_min=100, roto_stats=None, apply_to_all=False, bucket_mins=None, dollar_per_unit=None, to_csv=False, override=False, db=None, batter_year=""):
  totals = []
  abs = []
  hs = []
  rs = []
  ...
  for list_item in player_list:
    ...
    if eval_prv and roto_stats != None:
      ...
      if batter_year != "":
        load_to_db(db, p_ats, batter_year)
      if not apply_to_all:
...
def load_to_db(db, ats, batter_year):
    if not ats:
      print("BAD NAME + " + batter_year)
      return
    has_stats = db.query(BatterYearlyStats).filter(BatterYearlyStats.playerId == ats.name, BatterYearlyStats.year == batter_year).first()
    if has_stats:
      has_stats.playerId=ats.name,
      has_stats.year=batter_year,
      has_stats.team=ats.team,
      has_stats.hand=ats.hand,
      has_stats.gp=ats.gp,
      has_stats.bOrder=ats.batting_order,
      has_stats.pa=ats.pa,
      has_stats.ab=ats.ab,
      has_stats.h=ats.h,
      has_stats.r=ats.r,
      has_stats.hr=ats.hr,
      has_stats.rbi=ats.rbi,
      has_stats.bb=ats.bb,
      has_stats.sb=ats.sb,
      has_stats.cs=ats.cs,
      has_stats.pos=ats.pos,
      has_stats.rZ=ats.zscores.r,
      has_stats.hrZ=ats.zscores.hr,
      has_stats.rbiZ=ats.zscores.rbi,
      has_stats.sbZ=ats.zscores.sb,
      has_stats.avgZ=ats.zscores.hoa,
      has_stats.sumZ=ats.zscores.total
    else:
      has_stats = BatterYearlyStats(
        playerId=ats.name,
        year=batter_year,
        team=ats.team,
        hand=ats.hand,
        gp=ats.gp,
        bOrder=ats.batting_order,
        pa=ats.pa,
        ab=ats.ab,
        h=ats.h,
        r=ats.r,
        ...
        rZ=ats.zscores.r,
        hrZ=ats.zscores.hr,
        rbiZ=ats.zscores.rbi,
        sbZ=ats.zscores.sb,
        avgZ=ats.zscores.hoa,
        sumZ=ats.zscores.total
      )
      db.add(has_stats)

    db.commit()
    db.refresh(has_stats)
